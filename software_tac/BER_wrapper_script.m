...
function BER = runConvBlock(CHANNEL,modulation,noiseVar)
    ...
    for nEN=1:NEN
        ...
        elseif(strcmp(modulation, 'QPSK') == 1)
            h = (randn(frmLen * 3 / 2, 1) + 1j * randn(frmLen * 3 / 2, 1)) / sqrt(2);
        end
        reset(hError);
        reset(hconvde);
        for frmIdx = 1:NSlot
...
function [BERBlock,PretxBlock, PtxBlock] = runBlock(CHANNEL,modulation,noiseVar)
    ...
    for nEN=1:NEN
        hChanAWGN  = comm.AWGNChannel('NoiseMethod', 'Variance', 'Variance', noiseVar(nEN));
        if(strcmp(modulation, '16QAM') == 1)
            hBlock = (randn(nBlocks*15/4, 1) + 1j * randn(nBlocks*15/4, 1)) / sqrt(2);
        elseif(strcmp(modulation, 'QPSK') == 1)
            hBlock = (randn(nBlocks*15/2, 1) + 1j * randn(nBlocks*15/2, 1)) / sqrt(2);
        end
        reset(hError);
        reset(hconvde);
        for frmIdx = 1:NSlot
...
function [BERBlockConv,PretxBlockConv, PtxBlockConv] = runBlockConv(CHANNEL,modulation,noiseVar)
    ...
    for nEN=1:NEN
        hChanAWGN  = comm.AWGNChannel('NoiseMethod', 'Variance', 'Variance', noiseVar(nEN));
        if(strcmp(modulation, '16QAM') == 1)
            hBlockConv = (randn(nBlocks*15*3/4, 1) + 1j * randn(nBlocks*15*3/4, 1)) / sqrt(2);
        elseif(strcmp(modulation, 'QPSK') == 1)
            hBlockConv = (randn(nBlocks*15*3/2, 1) + 1j * randn(nBlocks*15*3/2, 1)) / sqrt(2);
        end
        reset(hError);
        reset(hconvde);
        for frmIdx = 1:NSlot
