...
%ascii coding
ascii = uint8(message);
encodedMessage_ascii  = reshape(de2bi(ascii, 8, 'left-msb').', 1, []);
...
for nn=1:NSlot
    ...
    bitaux = reshape(encodedMessage_ascii, 4, []).';
    bit1 = double(bitaux(:, 1));
    bit2 = double(bitaux(:, 2));
    bit3 = double(bitaux(:, 3));
    bit4 = double(bitaux(:, 4));
    ...
    message_all = zeros(NEN,len_ascii);
    for nEN=1:NEN
        Yk=zeros(N,L);
        for l=1:L
...
% end
%decoding 1e6 ascii
bytes_ascii = reshape(message_received, 8, []).';       % one row per byte
ascii_decoded = bin2dec(char(bytes_ascii + '0'));        % double column
decodedMessage = char(ascii_decoded).';
fprintf('\nFinal decoded message: |%s|\n',decodedMessage);

for nEN=1:NEN
    bytes_ascii = reshape(message_all(nEN,:), 8, []).';       % one row per byte
    ascii_decoded = bin2dec(char(bytes_ascii + '0'));        % double column
    decodedMessage = char(ascii_decoded).';
    fprintf('\n %d SNR --> Message decoded :|%s|\n', EN(nEN), decodedMessage);
end